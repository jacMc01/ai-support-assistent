...
function sentimentColor(s: string) {
  if (s === "Angry") return "text-red-600";
  if (s === "Frustrated") return "text-orange-500";
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-6xl px-5 py-10 sm:px-8 sm:py-12">
        <div className="mb-9">
          <p className="mb-3 text-xs font-semibold tracking-[0.18em] text-violet-600 uppercase">Your support workspace</p>
          <h1 className="text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">AI Support Assistant</h1>
        ...
        <div className="grid items-start gap-6 lg:grid-cols-[1.05fr_1fr]">
          ...
          <section className="panel p-6 sm:p-7" aria-labelledby="message-label">
            ...
            <form onSubmit={handleSubmit}>
              <label id="message-label" htmlFor="customer-message" className="mb-2.5 block text-sm font-medium text-slate-700">
                Customer Message
              </label>
              <textarea
                id="customer-message"
              ...
              <button
                ...
                className="mt-4 text-xs font-medium text-violet-600 transition hover:text-violet-800"
              >
              ...
              {error && (
                <p role="alert" className="mt-4 rounded-lg bg-red-50 p-3 text-xs leading-5 text-red-700">
                  {error}
                </p>
              )}
            </form>
          ...
          <div className="space-y-6">
            ...
            <section className="panel p-6 sm:p-7" aria-labelledby="analysis-title">
              <div className="mb-6 flex items-center justify-between gap-3">
                ...
                <span className="text-xs text-slate-400">{result ? "Complete" : "Awaiting request"}</span>
              </div>
              ...
              <h3 className="mt-6 text-sm font-medium text-slate-700">Summary</h3>
              <p className="mt-2 text-sm leading-6 text-slate-600">
                {result?.summary ?? <span className="text-slate-400">A concise summary of the customer&apos;s request will appear here.</span>}
              </p>
            </section>
            ...
            <section className="panel p-6 sm:p-7" aria-labelledby="response-title">
              ...
              {result?.response ? (
                <div className="rounded-xl border border-violet-100 bg-violet-50/40 p-5">
                  <span className="mb-3 block text-violet-500"><Sparkle /></span>
                  <p className="text-sm leading-7 text-slate-700 whitespace-pre-wrap">{result.response}</p>
                </div>
              ) : (
                <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50/50 p-5">
                  <span className="mb-3 block text-violet-400"><Sparkle /></span>
                  <p className="text-sm leading-7 text-slate-400">Your AI-assisted response will appear here, ready for you to review and personalize.</p>
                </div>
              )}
            </section>
          </div>
          ...
          <section className="panel p-6 sm:p-7 lg:col-span-2" aria-labelledby="review-title">
            ...
            <p className="mt-3 text-sm leading-6 text-slate-500">A second look at the response, across the qualities that matter.</p>
            <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-[repeat(4,1fr)_1.1fr]">
                ...
                return (
                  <div key={key} className="py-2">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className={`text-sm ${score != null ? "text-slate-700 font-medium" : "text-slate-400"}`} aria-label={score != null ? `${score} out of 100` : "Not scored"}>
                        {score != null ? score : "—"}
                      </span>
                    </div>
                    {score != null ? (
              ...
              <div className="rounded-xl bg-violet-50 px-5 py-4 sm:col-span-2 lg:col-span-1">
                <h3 className="text-xs font-medium text-violet-700">Overall Score</h3>
                <p className="mt-2 text-2xl font-semibold text-violet-600">
                  {result?.overallScore != null ? result.overallScore : <span className="text-violet-400">—</span>}
                  {" "}<span className="text-xs font-normal text-violet-400">/ 100</span>
                </p>
